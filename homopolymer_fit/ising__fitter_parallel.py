...
performs boostraps USING PARALLEL PROCESSING, calculates boostrap statistics 
and  parameter correlations, and plots histograms and parameter correlation 
plots.

Note, if you want to boostrap in parallel on multiple cores, set the numCores
parameter to the number of cores.  It is defaulted to one for single-core
processing.
...
proj_name = "cANK"
numCores = 4  # Cores to use for bootstrapping
...
def bootstrap(bootstrapiteration):
    rand_resid_dict = {}
    np.random.seed(bootstrapiteration)
    sys.stdout.flush()
    for melt in melts:
        rand_resid = []
        denat = melt_data_dict[melt][:, 0]  # A numpy array of type str
    ...
    BLparamslist = []
    bsresidual = bs_result.residual
    bsbest_chi = bs_result.chisqr
    bsred_chi = bs_result.redchi
    result = [
        bootstrapiteration,
        dGN,
        dGR,
...
if __name__ == "__main__":
    ...
    for construct in constructs:
        colorset = colorset + 1
        denat = construct1_data_dict[construct][
            :, 0
        ...
        norm_sig = norm_sig.astype(float)  # A numpy array of type float
        y_adj = baseline_adj(
            norm_sig, denat_line, result.params, construct + "_1"
        )
        y_fit = fit_model(result.params, denat_fit, construct + "_1")
        y_fit_adj = baseline_adj(
            y_fit, denat_fit, result.params, construct + "_1"
        )
        plt.plot(
            denat,
            y_adj,
    ...
    dGinter_vals = []
    mi_vals = []
    bootstrapList = [i + 1 for i in range(bs_iter_tot + 1)]
    print(f"Performing {bs_iter_tot} bootstrap iterations...")
    pool = mp.Pool(processes=numCores)  # Sets up a pool of processors
    BSParamValues = pool.map(
        bootstrap, bootstrapList
    )  # This is where the parallel bootstrapping occurs.  Function bootstrap() gets called the length
    # of bootstrapList times, with the values of bootstrapList as the argument in each call. The
    # tasks of calling bootstrap() that many times is split between the processors in the pool
    pool.close()
    pool.join()
    ...
    for i in BSParamValues:
        bs_param_values.append(i)
        dGN_vals.append(i[1])
        dGR_vals.append(i[2])
        dGC_vals.append(i[3])
        dGinter_vals.append(i[4])
        mi_vals.append(i[5])

    with open(
        os.path.join(PATH, f"{proj_name}_bootstrap_params.csv"), "w"
    ) as n:
        writer = csv.writer(n, delimiter=",")
    ...
    for param in bs_param_names:
        ...
        i = i + 1
    bs_statistics_df.to_csv(
        os.path.join(PATH, f"{proj_name}_bootstrap_stats.csv")
    )
    ...
    with PdfPages(os.path.join(PATH, f"{proj_name}_Corr_Plots.pdf")) as pdf:
        fig, axs = plt.subplots(
            ncols=gridsize, nrows=gridsize, figsize=(12, 12)
        )
        ...
        while y_param_counter < num_corr_params - 1:
            ...
            while x_param_counter < num_corr_params:
                ...
                if y_param_counter == 0:  # Puts labels above axes on top row
                    axs[
                        y_param_counter, x_param_counter
                ...
                axs[y_param_counter, x_param_counter].plot(

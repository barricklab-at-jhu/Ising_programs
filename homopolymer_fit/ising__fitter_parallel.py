...
#from os import system   # Uncomment this line and the last line in the script to have your mac announce when it's finished 
...
proj_name = 'cANK'
numCores = 1 #Cores to use for bootstrapping
...
def bootstrap(bootstrapiteration):
    rand_resid_dict = {}
    print ("Bootstrap iteration {0} out of {1}".format(bootstrapiteration, bs_iter_tot))
    ...
    result = [bootstrapiteration, dGN, dGR, dGC, dGinter, mi, bs_red_chisqr,bs_chisqr]
    sys.stdout.flush()
    return result    
...
if __name__ == '__main__':
    ...
    dGinter_vals = []
    mi_vals = []
    bootstrapList = []  # A list of integers to keep track on individual bootstraps
    for i in range(1, bs_iter_tot+1):
        bootstrapList.append(i) 
    pool = mp.Pool(processes=numCores) # Sets up a pool of processors
    BSParamValues = pool.map(bootstrap, bootstrapList) # This is where the parallel bootstrapping occurs.  Function bootstrap() gets called the length 
                                                       # of bootstrapList times, with the values of bootstrapList as the arguement in each call. The 
...
#system('say boot strapping is complete')